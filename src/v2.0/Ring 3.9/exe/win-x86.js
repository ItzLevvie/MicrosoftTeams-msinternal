...
https.get("https://config.teams.microsoft.com/config/v1/MicrosoftTeams/1415_1.0.0.0?environment=prod&audienceGroup=ring3_9&teamsRing=ring3_9&agent=TeamsBuilds", (getResponse) => {
    getResponse.on("data", (getData) => {
        try {
            if (getResponse.statusCode === 200) {
                const getBuild = JSON.parse(`${getData}`).BuildSettings.Desktop.windows.latestVersion;
