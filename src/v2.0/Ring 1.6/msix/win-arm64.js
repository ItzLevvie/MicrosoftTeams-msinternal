...
https.get("https://config.teams.microsoft.com/config/v1/MicrosoftTeams/1415_1.0.0.0?environment=life&audienceGroup=ring1_6&teamsRing=ring1_6&agent=TeamsBuilds", (getResponse) => {
    getResponse.on("data", (getData) => {
        try {
            if (getResponse.statusCode === 200) {
                const getBuild = JSON.parse(`${getData}`).BuildSettings.WebView2.arm64.latestVersion;
