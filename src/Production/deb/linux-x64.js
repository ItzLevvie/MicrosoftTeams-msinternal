...
const getPath = "repos/ms-teams/pool/main/t/teams";

https.get(getURL + "/" + getPath + "/", (getResponse) => {
    getResponse.on("data", (getData) => {
        if (getResponse.statusCode === 200) {
            const getLatestBuild = `${getData}`.match(/1.4.00.13653/);
            const getFile = "teams_" + getLatestBuild + "_amd64.deb";
            https.get(getURL + "/" + getPath + "/" + getFile, (getResponse) => {
                if (getResponse.statusCode === 200) {
                    ...
                    console.log(getPlatform + ": Sorry, but an error has occurred as we can not find a production build of Microsoft Teams. Please try again later.");
                }
            }).on("error", () => {
                if (enableLogging) {
                    console.log(getPlatform + ": Sorry, but an error has occurred as we can not connect to the host. Please try again later.");
                }
           });
        } else if (getResponse.statusCode === 404 && enableLogging) {
            console.log(getPlatform + ": Sorry, but an error has occurred as we can not find a production build of Microsoft Teams. Please try again later.");
