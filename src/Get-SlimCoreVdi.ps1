...
while ($maxAttempts -le 32) {
    try {
        ...
        #
        $baseUrl = "https://res.cdn.office.net/ic3-1/slimcorevdi"
        $rand = Get-Random
        if ($buildVersion) {
            curl https://statics.teams.cdn.office.net/production-windows-x64/$buildVersion/MSTeams-x64.msix --output MSTeams-x64-$rand.zip
            Expand-Archive .\MSTeams-x64-$rand.zip
            cd MSTeams-x64-$rand
            $a = (Get-ItemProperty -Path vdibridge_wv2.dll).VersionInfo.FileVersionRaw.ToString()
            cd ..
            $fileUrl = (Invoke-WebRequest -Uri "$baseUrl/$a/Microsoft.Teams.SlimCoreVdi.$Platform.msix" -Method Head)
            ...
            if ($runner -eq $true) {
                Write-Output "  Miscellaneous — SlimCoreVdi:"
            }
            Write-Output "  $a ($Platform) - published on $formatDate with $roundFileSize MB: $baseUrl/$a/Microsoft.Teams.SlimCoreVdi.$Platform.msix"
        }
        #
        break
    }
