...
const getFile = "Teams_windows.exe";

for (let getLatestBuild = 22465, getBuildLimit = getLatestBuild + 10; getLatestBuild <= getBuildLimit; getLatestBuild++) {
    https.get(getURL + "/" + getPath + "/" + getPartialBuild + getLatestBuild + "/" + getFile, (getResponse) => {
        if (getResponse.statusCode === 200) {
            ...
            console.log(getPartialBuild + getLatestBuild + " (" + getPlatform + "): Sorry, but an error has occurred as we can not find this continuous deployment build of Microsoft Teams. Please try again later.");
        }
    }).on("error", () => {
        if (enableLogging) {
            console.log(getPartialBuild + getLatestBuild + " (" + getPlatform + "): Sorry, but an error has occurred as we can not connect to the host. Please try again later.");
        }
    });
}
